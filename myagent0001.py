...
CLARIFY_REQUIREMENTS_SYSTEM_PROMPT = textwrap.dedent(
'''
You are a senior requirements clarifier. Your role is to clarify each rule in detail, especially when multiple rules are present.
Do not write code. Produce only a single, valid JSON object as output.

When identifying all possible input cases, you must account for every conceivable case, including those not explicitly mentioned in the instructions.

Mandates:
• Make all assumptions explicit so the spec covers all edge cases.
• Be concise but complete. Prioritize clarity and determinism over prose or cleverness.
...
"rules": a formal, ordered rules and requirements.
    (
        CRITICAL: Don't guess and make the rules yourself but only refer to and base on given information.
    )

"edge_cases": all edge cases that are not explicitly mentioned in the instructions.

Process to follow before producing JSON (internal reasoning, but reflect the results in the JSON fields):
• Understand the instruction carefully and identify all the requirements and rules.
...
def clarify_requirements(problem_statement: str) -> list:
    ...
    messages = [
        {"role": "system", "content": CLARIFY_REQUIREMENTS_SYSTEM_PROMPT},
    ...
    while True:
        ...
        while retry < 3:
            try:
                ...
                messages.append({"role": "user", "content": "Keep clarifying the requirements until you have a valid JSON object."})
...
def process_create_task(input_dict):
    ...
    if clarified_requirements and clarified_requirements.get("problem_type", "math") == "game":
    ...
    initial_solution = generate_initial_solution(problem_statement_with_spec, code_skeleton)
