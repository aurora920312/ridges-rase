...
class TreeOfThoughts:
    """Tree of Thoughts implementation for exploring multiple solution paths"""
    def __init__(self, max_depth=3, max_branches=3):
    ...
    def add_node(self, thought: str, tool_name: str, tool_args: dict, parent_id: str = None, depth: int = 0) -> str:
        node_id = str(uuid4())
        node = {
            "id": node_id,
            "thought": thought,
    ...
    def evaluate_node(self, node_id: str, observation: str) -> float:
        ...
        if "error" not in observation.lower():
            score += 0.3
        if "found" in observation.lower() or "located" in observation.lower():
            score += 0.4
        if "test passed" in observation.lower():
            score += 0.9
        if "saved successfully" in observation.lower():
            score += 0.6
        if "approved" in observation.lower():
            score += 0.7
        if "not found" in observation.lower():
            score -= 0.3
        if "syntax error" in observation.lower():
            score -= 0.6
    ...
    def get_next_node_to_explore(self) -> dict:
        unexplored_nodes = [n for n in self.nodes if n["status"] == "pending"]        
        if not unexplored_nodes:
            return None
        unexplored_nodes.sort(key=lambda x: (x["score"], -x["depth"]), reverse=True)
        shallow_nodes = [n for n in unexplored_nodes if n["depth"] < self.max_depth]        
        if shallow_nodes:
            return shallow_nodes[0]        
        return unexplored_nodes[0] if unexplored_nodes else None    
    
    def prune_weak_branches(self, min_score_threshold: float = 0.2):
        nodes_to_remove = []        
        for node in self.nodes:
            if (node["score"] < min_score_threshold and 
                node["status"] == "completed" and 
                len(node["children"]) == 0):
                nodes_to_remove.append(node["id"])        
        self.nodes = [n for n in self.nodes if n["id"] not in nodes_to_remove]
        for node in self.nodes:
            node["children"] = [child_id for child_id in node["children"] 
                              if child_id not in nodes_to_remove]    
    
    def get_path_to_node(self, node_id: str) -> list:
        path = []
        current_id = node_id        
        while current_id:
    ...
    def get_best_path(self) -> list:
        if self.best_solution_id:
            return self.get_path_to_node(self.best_solution_id)
        return []    
    
    def should_backtrack(self, current_score: float, step_count: int) -> bool:
        if step_count < 10:  # Don't backtrack too early
            return False
        if current_score < self.best_score - 0.3:
            return True
        if step_count > 50 and current_score < 0.3:
            return True        
        return False
...
class EnhancedCOT:
    class Action:
        ...
        def __init__(self, next_thought: str, next_tool_name: str, next_tool_args: dict, observation: list|tuple|str,is_error:bool=False,raw_response:str=None,total_attempts:int=0,inference_error_counter:dict=None,request_data:list=None):
            ...
            self.tot_node_id=None  # TOT tree node reference
            self.approach_score=0.0  # Score for this approach
    def __init__(self,latest_observations_to_keep=30,enable_tot=True):
        self.thoughts: list[EnhancedCOT.Action] = []
        self.latest_observations_to_keep=latest_observations_to_keep
        self.enable_tot = enable_tot
        self.tot_tree = TreeOfThoughts() if enable_tot else None
        self.exploration_mode = True
        self.exploration_steps = 0
        self.max_exploration_steps = 20
        self.best_approach_score = 0.0
        self.current_approach_id = None
    ...
    def add_action(self, action: EnhancedCOT.Action, inference_error_counter=None, request_data=None) -> bool: 
        self.thoughts.append(action)
        if self.enable_tot and self.tot_tree:
            if self.exploration_mode and self.exploration_steps < self.max_exploration_steps:
                node_id = self.tot_tree.add_node(
                    thought=action.next_thought,
                    tool_name=action.next_tool_name,
                    tool_args=action.next_tool_args,
                    parent_id=self.current_approach_id,
                    depth=len(self.thoughts)
                )
                action.tot_node_id = node_id
                self.current_approach_id = node_id
                self.exploration_steps += 1
                
                # Evaluate the action if we have an observation
                if action.observation:
                    score = self.tot_tree.evaluate_node(node_id, action.observation)
                    action.approach_score = score
                    if score > self.best_approach_score:
                        self.best_approach_score = score
                        
            # Switch to exploitation mode after exploration
            if self.exploration_steps >= self.max_exploration_steps:
                self.exploration_mode = False
                logger.info(f"[TOT] Switching to exploitation mode. Best score: {self.best_approach_score:.2f}")
    ...
    def should_backtrack(self, step_count: int) -> bool:
        if not self.enable_tot or not self.tot_tree:
            return False
        current_score = self.best_approach_score
        return self.tot_tree.should_backtrack(current_score, step_count)
        
    def get_alternative_approach(self) -> dict:
        if not self.enable_tot or not self.tot_tree:
            return None
            
        next_node = self.tot_tree.get_next_node_to_explore()
        if next_node:
            return {
                "thought": next_node["thought"],
                "tool_name": next_node["tool_name"],
                "tool_args": next_node["tool_args"],
                "score": next_node["score"]
            }
        return None
        
    def prune_tot_tree(self):
        if self.enable_tot and self.tot_tree:
            self.tot_tree.prune_weak_branches(min_score_threshold=0.2)
            logger.info(f"[TOT] Pruned tree. Remaining nodes: {len(self.tot_tree.nodes)}")    
            
    def get_tot_status(self) -> str:
        if not self.enable_tot or not self.tot_tree:
            return "TOT disabled"
        return f"Nodes: {len(self.tot_tree.nodes)}, Best Score: {self.tot_tree.best_score:.2f}, Mode: {'Exploration' if self.exploration_mode else 'Exploitation'}"
...
class EnhancedNetwork:
    ...
    @classmethod
    def is_valid_response(cls,raw_text)->bool:
        if type(raw_text) is dict and raw_text.get("error",None) is not None and raw_text.get("error")!="":
            return False,cls.ErrorType.EMPTY_RESPONSE.name
        if isinstance(raw_text, str) and not raw_text.strip().endswith("}") and not raw_text.strip().endswith("}]"):
        ...
        if isinstance(raw_text, str) and len(raw_text)==0:
            return False, cls.ErrorType.EMPTY_RESPONSE.name
        if isinstance(raw_text, str) and "<|reserved_token_" in raw_text:
            return False, cls.ErrorType.RESERVED_TOKEN_PRESENT.name
        if isinstance(raw_text, str) and 'API request failed with status 429' in raw_text:
            return False, cls.ErrorType.RATE_LIMIT_EXCEEDED.name
        if isinstance(raw_text, str) and 'Read timed out' in raw_text:
            return False, cls.ErrorType.TIMEOUT.name
        if isinstance(raw_text, str) and ('Network unreachable' in raw_text or 'Connection refused' in raw_text):
    ...
    def make_request(cls,messages:list,model:str,attempt:int=0, temperature:float=None)->str:
        if temperature is None:
            # Use auto-controller temperature if available
            if hasattr(cls, '_instance') and cls._instance and hasattr(cls._instance, 'temp_controller'):
                temperature = cls._instance.temp_controller.get_current_temperature()
            else:
                temperature = 0.0
        global run_id
...
class EnhancedToolManager:
    ...
    def tool(fn):
        def wrapper(self, *args, **kwargs):
            # Initialize tool tracking if not exists
            if not hasattr(self, 'tool_invocations'):
                self.tool_invocations = {}
            if not hasattr(self, 'tool_failure'):
                self.tool_failure = {}
            
            # Initialize counters for this tool if not exists
            ...
            try:
                ...
                if e.error_type not in self.tool_failure[fn.__name__]:
                    self.tool_failure[fn.__name__][e.error_type] = 0
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    @EnhancedToolManager.tool
    def explore_alternative_approach(self, current_approach: str, failure_reason: str) -> str:
        '''
        Explore alternative approaches when current one fails using Tree of Thoughts
        Arguments:
            current_approach: Description of current failing approach
            failure_reason: Why the current approach failed
        '''
        logger.info(f"[TOT] Exploring alternative to: {current_approach}")
        logger.info(f"[TOT] Failure reason: {failure_reason}")
        alternatives = []        
        if "not found" in failure_reason.lower():
            alternatives = [
                "Try searching with different keywords or patterns",
                "Look in different file types or directories", 
                "Use broader search terms",
                "Check for typos or different naming conventions"
            ]
        elif "syntax error" in failure_reason.lower():
            alternatives = [
                "Check the exact syntax requirements",
            ...
            alternatives = [
                ...
                "Use a more systematic debugging method",
                "Look for similar problems in the codebase"
            ]        
        return f"Alternative approaches to consider: {alternatives[:3]}"
    
    @EnhancedToolManager.tool
    def evaluate_approach_success(self, approach_description: str, results: str) -> str:
        '''
        Evaluate how successful an approach was using TOT scoring
        Arguments:
            approach_description: What approach was tried
            results: What results were obtained
        '''
        score = 0.0
        if "success" in results.lower():
            score += 0.8
        if "error" not in results.lower():
            score += 0.3
        if "found" in results.lower() or "located" in results.lower():
            score += 0.4
        if "test passed" in results.lower():
            score += 0.9
        if "saved successfully" in results.lower():
            score += 0.6
        if "approved" in results.lower():
            score += 0.7
        if "not found" in results.lower():
            score -= 0.3
        if "syntax error" in results.lower():
            score -= 0.6
        if "timeout" in results.lower():
            score -= 0.8
        if "failed" in results.lower():
            score -= 0.5
        if score >= 0.7:
            recommendation = "Continue with this approach - it's working well"
        elif score >= 0.4:
            recommendation = "This approach shows promise, but may need refinement"
        elif score >= 0.1:
            recommendation = "Consider trying an alternative approach"
        else:
            recommendation = "This approach is not working, try something different"        
        return f"Approach '{approach_description}' scored {score:.2f}/1.0. {recommendation}. Results: {results}"
    
    @EnhancedToolManager.tool
    def backtrack_to_better_approach(self, current_progress: str) -> str:
        '''
        Backtrack to a better approach when current one is not working well
        Arguments:
            current_progress: Summary of current progress and issues
        '''
        logger.info(f"[TOT] Backtracking due to poor progress: {current_progress}")        
        backtrack_strategies = [
            "Go back to the problem analysis phase and re-examine the requirements",
            "Try a different search strategy with broader or different keywords",
            "Focus on a different file or component that might be related",
            "Use a more systematic approach to debug the issue",
            "Look for similar problems in the codebase and see how they were solved"
        ]        
        return f"Backtracking strategies: {backtrack_strategies[:3]}"
    
    @EnhancedToolManager.tool
    def parallel_codebase_analysis(self, file_path: str, test_func_names: List[str] = None) -> str:
        '''
        Execute multiple analysis tools in parallel for faster results
        Arguments:
            file_path: Path to the file to analyze
            test_func_names: List of test function names to focus on
        '''
        try:
            executor = ParallelToolExecutor(self, max_workers=4)
            results = executor.execute_parallel_analysis(file_path, test_func_names or [])
            
            analysis_summary = "Parallel Analysis Results:\n"
            for task_name, result in results.items():
                # Safely slice the result string
            ...
            return f"Parallel analysis failed: {str(e)}"
    
    @EnhancedToolManager.tool
    def get_temperature_stats(self) -> str:
        '''
        Get current temperature and performance statistics
        '''
        try:
            if hasattr(EnhancedNetwork, '_instance') and EnhancedNetwork._instance:
                stats = EnhancedNetwork._instance.temp_controller.get_performance_stats()
                return f"Temperature Stats: {stats}"
            else:
                return "Temperature controller not available"
        except Exception as e:
            return f"Failed to get temperature stats: {str(e)}"
    
    @EnhancedToolManager.tool
    def force_temperature(self, temperature: float) -> str:
        '''
        Manually set the temperature for LLM requests
        Arguments:
            temperature: Temperature value (0.0-1.0)
        '''
        try:
            if hasattr(EnhancedNetwork, '_instance') and EnhancedNetwork._instance:
                EnhancedNetwork._instance.temp_controller.force_temperature(temperature)
    ...
    def get_cache_stats(self) -> str:
        ...
        '''
        try:
            # This would need to be implemented with a global cache instance
            return "Cache statistics: Not implemented yet"
        except Exception as e:
            return f"Failed to get cache stats: {str(e)}"
...
def fix_task_solve_workflow(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "", \
    ...
    # Initialize EnhancedCOT with TOT and extended memory
    cot=EnhancedCOT(latest_observations_to_keep=30, enable_tot=True)
    
    # Initialize EnhancedNetwork with temperature controller
    EnhancedNetwork._instance = EnhancedNetwork()
    ...
    tool_manager=FixTaskEnhancedToolManager(
        available_tools=[
            ...
            "evaluate_approach_success", 
            "backtrack_to_better_approach",
    ...
    for step in range(n_max_steps):
        ...
        # TOT Status Monitoring
        logger.info(f"[TOT] {cot.get_tot_status()}")
        
        # Automatic TOT pruning every 20 steps
        if step % 20 == 0 and step > 0:
            cot.prune_tot_tree()
            
        # TOT Backtracking Logic
        if cot.should_backtrack(step):
            logger.info(f"[TOT] Backtracking triggered at step {step}")
            alternative = cot.get_alternative_approach()
            if alternative:
                logger.info(f"[TOT] Switching to alternative approach: {alternative['thought']}")
                messages.append({"role": "user", "content": f"TOT Backtrack: Consider switching to this alternative approach: {alternative['thought']}"})
        ...
        try:
            next_thought, next_tool_name, next_tool_args,raw_text,total_attempts,error_counter,messages = EnhancedNetwork.inference(messages, model=GLM_MODEL_NAME, run_id=run_id, temperature=EnhancedNetwork._instance.temp_controller.get_current_temperature())
            
            # Track performance for temperature adjustment
            success = not (isinstance(raw_text, dict) and raw_text.get("error", False))
            EnhancedNetwork._instance.temp_controller.record_performance(success)
        except Exception as e:
            import traceback  # Ensure traceback is accessible
    ...
    # Final TOT Status
    if cot.enable_tot and cot.tot_tree:
        logger.info(f"[TOT] Final status: {cot.get_tot_status()}")
        logger.info(f"[TOT] Total nodes explored: {len(cot.tot_tree.nodes)}")
        logger.info(f"[TOT] Best approach score: {cot.best_approach_score:.2f}")
    
    # Final Temperature Stats
    if hasattr(EnhancedNetwork, '_instance') and EnhancedNetwork._instance:
        temp_stats = EnhancedNetwork._instance.temp_controller.get_performance_stats()
        logger.info(f"[TEMP] Final temperature: {temp_stats['current_temperature']:.4f}")
        logger.info(f"[TEMP] Consecutive successes: {temp_stats['consecutive_successes']}")
        logger.info(f"[TEMP] Consecutive errors: {temp_stats['consecutive_errors']}")
