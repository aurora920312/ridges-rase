...
AGENT_MODELS = [QWEN_MODEL_NAME, KIMI_MODEL_NAME, DEEPSEEK_MODEL_NAME, GLM_MODEL_NAME]
        ...
        try:
            ...
            if solution:
                ...
                if retry == 0:
                    ...
                    messages.append(
                        {"role": "user", "content": f"""Act as a Python code expert. This is a initial solution for this problem. Check it and if it has some errors or bugs you think, fix it.\n\nGenerate the complete and correct implementation in python files."""},
...
def extract_and_write_files(initial_solution: str, base_dir: str = ".") -> list:
    ...
    return created_files
def generate_test_cases_from_problem(problem_statement: str, code_skeleton: str) -> str:
    """
    Generate comprehensive test cases from problem statement using LLM.
    Tests include detailed debug output for clear failure analysis.
    Enhanced to generate more comprehensive and complex test cases.
    """
    try:
        messages = [
            {
                "role": "system",
                "content": """You are an expert test engineer. Generate comprehensive unittest test cases based on the problem statement.

CRITICAL REQUIREMENTS:
1. Generate AT LEAST 15-20 test cases covering ALL scenarios
2. Include debug print statements in EVERY test for clear failure analysis
3. Use EXACT values from problem statement examples (if provided)
4. Test ALL edge cases mentioned in the problem
5. Test boundary conditions: empty inputs, single items, maximum values
6. Test complex cases with multiple valid solutions or optimization scenarios
7. Test random large inputs to verify algorithm correctness
8. Use descriptive test names that explain what is being tested
9. Add detailed assertions with clear error messages
10. Output ONLY valid Python unittest code

COMPLEXITY MATCHING:
- If problem mentions "optimization" or "minimum/maximum", test multiple valid groupings
- If problem has specific examples with numbers, use THOSE EXACT numbers in tests
- If problem mentions special cases, create dedicated tests for each
- Generate stress tests with large inputs (e.g., 100+ items)

DEBUG OUTPUT REQUIREMENT:
Every test MUST include these prints:
- print(f"[TEST] Testing: description")
- print(f"[DEBUG] Input: {{input}}")
- print(f"[DEBUG] Output: {{actual_result}}")
...
class TestSolution(unittest.TestCase):
    def test_descriptive_name(self):
        print(f"[TEST] Testing: descriptive name")
...
Generate comprehensive test cases with debug output for all scenarios in the problem statement.

IMPORTANT:
- Generate AT LEAST 15-20 test cases
- Use EXACT examples from the problem statement with their exact expected values
- Cover ALL edge cases: empty, single item, maximum complexity
- Include stress tests with large inputs
- Test any optimization scenarios thoroughly
- Make tests as comprehensive as the real test suite would be"""
            }
        ]
        ...
        logger.info("[FEEDBACK-LOOP] Generated test cases")
        return test_code
        
    except Exception as e:
        logger.error(f"[FEEDBACK-LOOP] Error generating tests: {e}")
        return ""
def validate_test_quality(test_code: str) -> dict:
    """
    Validate that generated tests are comprehensive enough.
    Returns: {"is_valid": bool, "warnings": list, "test_count": int}
    """
    warnings = []
    ...
    if test_count < 10:
        warnings.append(f"Only {test_count} tests generated (recommended: 15-20)")
    
    # Check for debug prints
    if "[DEBUG]" not in test_code or test_code.count("[DEBUG]") < test_count * 2:
        warnings.append("Insufficient debug print statements in tests")
    
    # Check for [TEST] markers
    if test_code.count("[TEST]") < test_count:
        warnings.append("Missing [TEST] markers in some test methods")
    
    # Check for assertions
    if test_code.count("assert") < test_count:
        warnings.append("Some tests may be missing assertions")
    
    # Check for edge case tests
    edge_case_keywords = ["empty", "single", "zero", "none", "boundary", "maximum", "minimum", "large"]
    edge_case_found = any(keyword in test_code.lower() for keyword in edge_case_keywords)
    if not edge_case_found:
        warnings.append("No obvious edge case tests found (empty, boundary, etc.)")
    
    is_valid = test_count >= 5  # Minimum viable threshold
...
def run_tests_and_get_feedback(test_file: str = "generated_tests.py") -> tuple[bool, str]:
    """Run tests and return (success, detailed_feedback)"""
    try:
        ...
        logger.info(f"[FEEDBACK-LOOP] Tests: {'✅ PASSED' if success else '❌ FAILED'}")
...
def regenerate_solution_with_feedback(problem_statement: str, code_skeleton: str, 
                                      previous_solution: str, all_test_history: list, 
                                      iteration: int) -> str:
    """
    Regenerate solution based on CUMULATIVE test failure history.
    all_test_history: List of (iteration_num, test_feedback) tuples
    """
    try:
        # Build cumulative history section
        history_section = "## DEBUGGING HISTORY (Learn from ALL previous attempts):\n\n"
        for iter_num, feedback in all_test_history:
            history_section += f"### Iteration {iter_num} Test Results:\n"
            history_section += f"{feedback}\n"
            history_section += f"{'='*60}\n\n"
        
        messages = [
            {
                "role": "system",
                "content": """You are an expert Python developer fixing a failed solution.

CRITICAL: You have access to ALL previous test results. Learn from the PATTERN of failures:
1. See what you already tried and why it failed
2. Don't repeat the same mistakes
3. Identify if you're making progress (e.g., fixed type error, now fixing logic)
4. Use debug output to understand exact failure points
5. Generate a solution that addresses ALL observed issues
...
Analyze the COMPLETE history above and generate a corrected solution that addresses ALL patterns of failure."""
            }
        ]
        ...
        logger.info(f"[FEEDBACK-LOOP] Regenerated (iteration {iteration}) with {len(all_test_history)} historical feedbacks")
        return solution
        
    except Exception as e:
        logger.error(f"[FEEDBACK-LOOP] Regeneration error: {e}")
        return ""

def _perform_additional_error_checks(problem_statement: str, solution: str) -> str:
    """
    Performs additional error prevention checks and fixes.
    ...
    Returns:
        str: The solution with additional fixes applied
    """
    try:
        # Check for common error patterns
        if "class " not in solution:
            logger.info("[ADDITIONAL_CHECKS] No class found, skipping additional checks")
            return solution
            
        # Basic validation checks
        if not solution.strip():
            logger.warning("[ADDITIONAL_CHECKS] Empty solution detected")
            return solution
            
        # Check for potential syntax issues
        if solution.count('(') != solution.count(')'):
            logger.warning("[ADDITIONAL_CHECKS] Mismatched parentheses detected")
            
        if solution.count('[') != solution.count(']'):
            logger.warning("[ADDITIONAL_CHECKS] Mismatched brackets detected")
            
        if solution.count('{') != solution.count('}'):
            logger.warning("[ADDITIONAL_CHECKS] Mismatched braces detected")
        
        logger.info("[ADDITIONAL_CHECKS] Additional error checks completed")
        return solution
        
    except Exception as e:
        logger.error(f"[ADDITIONAL_CHECKS] Additional error checks failed: {e}")
        return solution

def fix_dynamic_property_initialization(problem_statement: str, initial_solution: str) -> str:
    """Fix dynamic property initialization issues using LLM analysis for specific problems only."""
    try:
        ...
        # Use LLM to determine if this problem needs dynamic property initialization fix
        detection_messages = [
            {
                "role": "system",
                "content": "You are a Python code expert. Analyze if this problem involves random generation, unique value creation, or dynamic content generation that needs proper state management and entropy handling. Look for patterns like random names, IDs, tokens, or any generated content that must be unique. Return only 'YES' or 'NO'."
            },
            {
                "role": "user", 
                "content": f"Problem statement:\n{problem_statement}\n\nCurrent solution:\n{initial_solution}\n\nDoes this problem involve random generation, unique value creation, or dynamic content generation that requires proper state management and entropy handling?"
            }
        ]
        ...
        if not needs_fix:
            logger.info("[DYNAMIC_PROPERTY_FIX] Problem does not need dynamic property initialization fix, skipping")
            return initial_solution
            
        logger.info("[DYNAMIC_PROPERTY_FIX] Problem needs dynamic property initialization fix, applying LLM-based fix")
        
        messages = [
            {
                "role": "system",
                "content": "You are a senior Python developer specializing in code quality and design. Your task is to improve the code by implementing proper lazy initialization and ensuring clean separation of concerns. Focus on making the code more maintainable, testable, and following Python best practices. Return ONLY the complete corrected Python code with filename. Do not include any explanations, comments, or markdown formatting."
            },
            {
                "role": "user", 
                "content": f"Problem statement:\n{problem_statement}\n\nCurrent solution:\n{initial_solution}\n\nPlease review and improve this code by:\n1. Implementing proper lazy initialization where appropriate\n2. Ensuring instance-level state management instead of class-level\n3. Following clean code principles and Python conventions\n4. Making the code more robust and maintainable\n5. Ensuring proper encapsulation and data hiding\n6. Ensuring that functionality works correctly with deterministic testing\n7. Using instance-level tracking for used names to avoid conflicts between different instances\n8. Making sure each instance maintains its own state independently\n9. Adding sufficient entropy to random generation to ensure uniqueness across different calls\n10. Using time-based or instance-based randomization to prevent identical outputs with same seeds\n\nCRITICAL FOR RANDOM GENERATION:\n- Replace any random generation functions that could produce repeated results\n- Use time-based entropy to ensure unique results across calls\n- Apply creative time-based randomization\n- Maintain the same return type and function signature\n\nReturn ONLY the corrected Python code with filename, no explanations."
            }
        ]
        
        response = EnhancedNetwork.make_request(messages, model=QWEN_MODEL_NAME)
        logger.info(f"[DYNAMIC_PROPERTY_FIX] LLM response received ({len(response)} chars)")
        ...
        logger.info(f"[DYNAMIC_PROPERTY_FIX] Fixed solution first line: '{first_line}'")
        logger.info(f"[DYNAMIC_PROPERTY_FIX] Fixed solution length: {len(fixed_solution)} chars")
        
        if first_line.endswith('.py'):
            logger.info("[DYNAMIC_PROPERTY_FIX] Successfully applied LLM-based dynamic property initialization fix")
            return fixed_solution
        else:
            logger.warning(f"[DYNAMIC_PROPERTY_FIX] Format validation failed - first line: '{first_line}', using original")
            return initial_solution
            
    except Exception as e:
        logger.error(f"[DYNAMIC_PROPERTY_FIX] LLM-based fix failed: {e}, using original")
        return initial_solution

def fix_circular_dependency_issues(problem_statement: str, initial_solution: str) -> str:
    """Fix circular dependency issues in evaluator using LLM analysis."""
    try:
        # Check if solution contains a class definition
        if "class " not in initial_solution:
            logger.info("No class found in solution, skipping")
        ...
        # Use LLM to determine if this problem needs circular dependency fix
        detection_messages = [
            {
                "role": "system",
                "content": "You are a programming expert. Analyze if this problem requires fixing circular dependency issues. Look for code that handles definitions that can reference themselves, causing infinite loops during execution. Return only 'YES' or 'NO'."
            },
            {
                "role": "user", 
                "content": f"Problem statement:\n{problem_statement}\n\nCurrent solution:\n{initial_solution}\n\nDoes this problem have circular dependency issues where definitions can reference themselves, causing infinite loops during execution-time expansion?"
            }
        ]
        ...
        if not needs_fix:
            logger.info("[CIRCULAR_DEPENDENCY_FIX] Problem does not need circular dependency fix, skipping")
            return initial_solution
            
        logger.info("[CIRCULAR_DEPENDENCY_FIX] Circular dependency patterns detected, applying LLM-based fix")
        
        messages = [
            {
                "role": "system",
                "content": "You are a programming expert. Fix circular dependency issues in the evaluator. The problem is that when a definition references itself, the current expansion logic creates infinite loops. Fix this by properly handling circular dependencies during definition time, not execution time. IMPORTANT: Do NOT prevent circular dependencies - they should be allowed and handled correctly. CRITICAL: Return ONLY the corrected Python code with filename, no explanations, comments, or markdown formatting. Do not use ```python or ``` blocks."
            },
            {
                "role": "user", 
                "content": f"Problem statement: {problem_statement}\n\nCurrent solution: {initial_solution}\n\nFix the circular dependency issue. Follow these steps: STEP 1: PROBLEM - Current solution has infinite loops when definitions reference themselves - Must handle circular dependencies properly STEP 2: REQUIREMENTS - Circular dependencies must be ALLOWED and handled correctly - Do NOT raise errors for circular dependencies - Do NOT add conditions that prevent self-reference STEP 3: SOLUTION - Process definitions during definition time, not execution time - Create flattened version by expanding references - If reference points to same word being defined, keep it as-is - If reference points to different word, expand it recursively STEP 4: EXECUTION - When encountering defined word, prepend its flattened definition to input stream - Use simple prepending approach - Do NOT replace entire token list or reset index STEP 5: ARCHITECTURE - Process definitions line by line first, then execute only the last line - Separate definition phase from execution phase completely - Use simple prepending approach - Do NOT use complex token list manipulation or index reset STEP 6: VALIDATION - Check if word name is a number (positive or negative) using try/except approach - If conversion succeeds, it's a number - raise error immediately - If conversion fails, it's not a number - continue normally - Use simple logic: try conversion, if successful then raise error, if exception then continue Return ONLY the corrected Python code with filename, no explanations, no markdown formatting, no ``` blocks."
            }
        ]
        
        response = EnhancedNetwork.make_request(messages, model=QWEN_MODEL_NAME)
        logger.info(f"[CIRCULAR_DEPENDENCY_FIX] LLM response received ({len(response)} chars)")
        ...
        logger.info(f"[CIRCULAR_DEPENDENCY_FIX] Fixed solution first line: '{first_line}'")
        logger.info(f"[CIRCULAR_DEPENDENCY_FIX] Fixed solution length: {len(fixed_solution)} chars")
        
        if first_line.endswith('.py'):
            logger.info("[CIRCULAR_DEPENDENCY_FIX] Successfully applied LLM-based circular dependency fix")
            return fixed_solution
        else:
            logger.warning(f"[CIRCULAR_DEPENDENCY_FIX] Format validation failed - first line: '{first_line}', using original")
            return initial_solution
            
    except Exception as e:
        logger.error(f"[CIRCULAR_DEPENDENCY_FIX] LLM-based fix failed: {e}, using original")
        return initial_solution
    
def prevent_potential_errors(problem_statement: str, initial_solution: str) -> str:
    """
    Comprehensive error prevention system that analyzes and fixes potential issues in generated solutions.
    
    This function integrates multiple error detection and correction mechanisms:
    - Dynamic property initialization issues
    - Circular dependency problems
    - General code quality improvements
    
    Args:
        problem_statement: The original problem description
        initial_solution: The generated solution to analyze and fix
        
    Returns:
        str: The improved solution with potential errors fixed
    """
    logger.info("[ERROR_PREVENTION] Starting comprehensive error prevention analysis")
    
    try:
        # Start with the original solution
        improved_solution = initial_solution
        # Step 2: Fix circular dependency issues
        logger.info("[ERROR_PREVENTION] Step 2: Analyzing circular dependency issues")
        improved_solution = fix_circular_dependency_issues(problem_statement, improved_solution)
        
        # Step 1: Fix dynamic property initialization issues
        logger.info("[ERROR_PREVENTION] Step 1: Analyzing dynamic property initialization")
        improved_solution = fix_dynamic_property_initialization(problem_statement, improved_solution)
        
        # Step 3: Additional error prevention checks
        logger.info("[ERROR_PREVENTION] Step 3: Performing additional error prevention checks")
        improved_solution = _perform_additional_error_checks(problem_statement, improved_solution)
        
        logger.info("[ERROR_PREVENTION] Error prevention analysis completed successfully")
...
def process_create_task(input_dict):
    ...
    # Comprehensive error prevention and fixing
    initial_solution = prevent_potential_errors(problem_statement, initial_solution)
...
#     # PHASE 1: Generate test cases from problem statement
#     logger.info("[FEEDBACK-LOOP] Phase 1: Generating comprehensive test cases...")
#     test_code = generate_test_cases_from_problem(problem_statement, code_skeleton)
    
#     if not test_code:
#         logger.warning("[FEEDBACK-LOOP] No tests generated, falling back to simple generation")
#         initial_solution = generate_initial_solution(problem_statement, code_skeleton)
#         initial_solution = enhance_solution_with_constants(problem_statement, initial_solution)
#         initial_solution = fix_lazy_property_generation(problem_statement, initial_solution)
#         initial_solution = fix_recursive_word_definitions(problem_statement, initial_solution)
...
#     validation = validate_test_quality(test_code)
#     logger.info(f"[FEEDBACK-LOOP] Test validation: {validation['test_count']} tests generated")
#     if validation['warnings']:
#         for warning in validation['warnings']:
#             logger.warning(f"[FEEDBACK-LOOP] Test quality warning: {warning}")
    
#     if not validation['is_valid']:
#         logger.error("[FEEDBACK-LOOP] Generated tests failed quality validation, falling back")
#         initial_solution = generate_initial_solution(problem_statement, code_skeleton)
#         initial_solution = enhance_solution_with_constants(problem_statement, initial_solution)
#         initial_solution = fix_lazy_property_generation(problem_statement, initial_solution)
#         initial_solution = fix_recursive_word_definitions(problem_statement, initial_solution)
...
#         f.write(test_code)
#     logger.info(f"[FEEDBACK-LOOP] ✅ Wrote validated test file: {test_file} ({validation['test_count']} tests)")
    
#     # PHASE 2: Generate initial solution
#     logger.info("[FEEDBACK-LOOP] Phase 2: Generating initial solution...")
#     current_solution = generate_initial_solution(problem_statement, code_skeleton)
#     current_solution = enhance_solution_with_constants(problem_statement, current_solution)
#     current_solution = fix_lazy_property_generation(problem_statement, current_solution)
#     current_solution = fix_recursive_word_definitions(problem_statement, current_solution)
    
#     print(current_solution)
    
#     # PHASE 3: Feedback loop - iterate until tests pass
#     MAX_ITERATIONS = 12  # Increased for better convergence while maintaining reasonable cost
#     iteration = 0
#     test_history = []  # Accumulate ALL test results: [(iteration, feedback), ...]
    
#     while iteration < MAX_ITERATIONS:
#         iteration += 1
#         logger.info(f"[FEEDBACK-LOOP] Phase 3: Iteration {iteration}/{MAX_ITERATIONS} (enhanced feedback loop)")
        
#         # Write solution files
#         created_files = extract_and_write_files(current_solution)
#         print(f"[Iteration {iteration}] Created {len(created_files)} files: {created_files}")
        
#         # Run tests
#         success, feedback = run_tests_and_get_feedback(test_file)
        
#         # Store this result in history
#         test_history.append((iteration, feedback))
        
#         if success:
#             elapsed = time.time() - start_time
#             logger.info(f"[FEEDBACK-LOOP] ✅ SUCCESS after {iteration} iteration(s) in {elapsed:.1f}s!")
#             print(f"[Iteration {iteration}] ✅ ALL TESTS PASSED!")
#             print(f"{feedback}")
#             break
#         else:
#             logger.warning(f"[FEEDBACK-LOOP] ❌ Iteration {iteration} failed")
#             print(f"\n{'='*70}")
#             print(f"[Iteration {iteration}] ❌ TEST FAILURE - Full Output:")
#             print(f"{'='*70}")
#             print(feedback)  # Print FULL feedback, not truncated!
#             print(f"{'='*70}\n")
            
#             if iteration < MAX_ITERATIONS:
#                 logger.info(f"[FEEDBACK-LOOP] Regenerating with CUMULATIVE history ({len(test_history)} attempts)...")
#                 current_solution = regenerate_solution_with_feedback(
#                     problem_statement,
#                     code_skeleton,
#                     current_solution,
#                     test_history,  # Pass ENTIRE history
#                     iteration
#                 )
#                 current_solution = enhance_solution_with_constants(problem_statement, current_solution)
#                 current_solution = fix_lazy_property_generation(problem_statement, current_solution)
...
#                     logger.error("[FEEDBACK-LOOP] Regeneration failed")
#                     break
#             else:
#                 logger.warning(f"[FEEDBACK-LOOP] Max iterations reached")
    
#     # Clean up test file (don't include in patch)
