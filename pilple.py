...
PROBLEM_TYPE_SYSTEM_PROMPT = textwrap.dedent(
'''
You are a senior algorithm assistant. Your role is to determine the algorithm type of given problem and assist the user to implement the algorithm correctly and simply.
Do not write code. Produce only a single, valid JSON object as output.

Output requirements:
• Output must be one valid JSON object, with no commentary outside the JSON.
• The JSON must include the following top-level keys and semantics:

"problem_type": The type of the problem in algorithmic concept.
    (
        i.g. graph, game, data_structure, string, math, etc.
    )

"algorithm_guide": concise and actionable guidance for implementing the correct and efficient algorithm, ensuring simplicity and accuracy based on the problem statement, and avoiding unnecessary complexity that could lead to incorrect solutions.

"edge_cases": a comprehensive list of all relevant edge cases derived from the problem statement. Don't think any exploit input cases.

Process to follow before producing JSON (internal reasoning, but reflect the results in the JSON fields):
• Understand the problem statement deeply and identify all the requirements.
• Identify the type of the problem in algorithmic concept. (i.g. graph, game, data_structure, string, math, etc.)
• Ensure all above mentioned keys are included.
...
ALGORITHM_ASSISTANT_PROMPTS = {
    "game": "You must clarify each rule in detail, especially when multiple rules are present.",
    "graph": """
    The most important thing is to build the graph correct.
    The first thing you have to do is understand the input correctly and convert it as correct graph.
    """
}
...
def generate_solution_with_multi_step_reasoning(problem_statement: str, code_skeleton: str, problem_type: str) -> str:
    retry = 0
    
    global GENERATE_SOLUTION_WITH_MULTI_STEP_REASONING_PROMPT
    if problem_type:
        GENERATE_SOLUTION_WITH_MULTI_STEP_REASONING_PROMPT += ALGORITHM_ASSISTANT_PROMPTS.get(problem_type, "")
...
def generate_initial_solution(problem_statement: str, code_skeleton: str, problem_type: str) -> str:
    retry = 0
    while retry < 10:
        try:
            logger.info("Starting multi-step reasoning solution generation")
            
            solution = generate_solution_with_multi_step_reasoning(problem_statement, code_skeleton, problem_type)
...
def clarify_requirements(problem_statement: str) -> list:
    ...
    messages = [
        {"role": "system", "content": PROBLEM_TYPE_SYSTEM_PROMPT.format(problem_statement=problem_statement)},
    ...
    while True:
        ...
        while retry < 3:
            try:
                ...
                clarified_requirements = json.loads(response)
                
                is_valid, error_msg = validate_response(clarified_requirements)
                print(error_msg)
                if is_valid:
                    return clarified_requirements
                messages.append({"role": "assistant", "content": response})
                messages.append({"role": "user", "content": "Keep clarifying the problem type until you have a valid JSON object."})
...
def process_create_task(input_dict):
    ...
    clarified_requirements = clarify_requirements(problem_statement)
    print("clarified_requirements: ", clarified_requirements)
    problem_type = clarified_requirements.get("problem_type", "math")
    print("problem_type: ", problem_type)
    
    if clarified_requirements and problem_type == "graph":
        print("DOING GAME AGLRORITHM")
        if isinstance(clarified_requirements, str):
            problem_statement_with_spec += f"""
            Implement the functions so that all of the following requirements and rules are satisfied, strictly preserving the exact order in which they are listed.
            Spec:
            {clarified_requirements}
            """
        else:
            ...
            Implement the functions so that all of the following requirements and rules are satisfied, strictly preserving the exact order in which they are listed.
            Spec:
            {json.dumps(clarified_requirements, indent=4)}
            """

    # exit()
    code_skeleton = get_code_skeleton()
    start_time = time.time()
    initial_solution = generate_initial_solution(problem_statement, code_skeleton, problem_type)
    ...
    test_cases = generate_test_files(problem_statement_with_spec, created_files, code_skeleton)
    print(test_cases)
