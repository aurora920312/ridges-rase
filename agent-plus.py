...
## Enhanced Test Discovery & Execution:
- The system now supports both unittest and pytest frameworks with automatic detection
- Test discovery includes files with patterns: test_*.py, *_test.py, tests.py, unittest_*.py, *_spec.py
- Framework detection analyzes file content to determine unittest vs pytest usage
- Test execution automatically adapts to the detected framework with robust fallback mechanisms
- Configuration files (pytest.ini, pyproject.toml, setup.cfg, tox.ini, conftest.py) are checked first
- Multiple execution strategies ensure tests run successfully across different environments
...
TIMEOUT_ALLOCATION = {
    ...
    'SINGLE_TOOL_TIMEOUT': 90,        # Max time per tool call
    'MODEL_UPGRADE_TRIGGER': 400,     # Upgrade model after 6.7 minutes
    'PROGRESS_CHECK_INTERVAL': 120,   # Check progress every 2 minutes
    'SUBPROCESS_TIMEOUT': 60,         # Git/pytest operations
    'HTTP_REQUEST_TIMEOUT': 90,       # API calls
}
...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def run_repo_tests(self,file_paths:List[str])->str:
        '''
        Runs the tests for the repository with enhanced framework detection and robust execution.
        Arguments:
            file_paths: path of the files to run the tests for.
        Output:
            Returns the stdout/stderr from the executed files with framework-specific execution.
        '''
        logger.info(f"Running tests for {len(file_paths)} files: {file_paths}")
        results = []
        ...
        for file_path in file_paths:
            ...
            framework = detect_test_framework(file_path)
            logger.info(f"Detected framework for {file_path}: {framework}")
            
            # Try framework-specific execution
            success = False
            
            if framework == "unittest":
                success = self._run_unittest_tests(file_path, results)
            elif framework == "pytest":
                success = self._run_pytest_tests(file_path, results)
            
            # If framework-specific execution failed or framework is unknown, try fallback
            if not success:
                logger.info(f"Framework-specific execution failed for {file_path}, trying fallback methods")
                self._run_fallback_tests(file_path, results)
        
        # If no results from individual file execution, try the original method
        if not results:
            logger.info("No individual file results, falling back to original test runner method")
            return self._run_original_test_method(file_paths)
        
        return "\n".join(results)
    
    def _run_unittest_tests(self, file_path: str, results: List[str]) -> bool:
        """Run unittest tests for a specific file."""
        try:
            # Try python -m unittest with module path
            module_path = filepath_to_module(file_path, os.getcwd(), "")
            cmd = ["python", "-m", "unittest", module_path]
            logger.info(f"Running unittest command: {' '.join(cmd)}")
            
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=90)
            output = f"unittest ({file_path}):\n{result.stdout}{result.stderr}"
            results.append(output)
            
            if result.returncode == 0:
                logger.info(f"unittest execution successful for {file_path}")
                return True
            else:
                logger.warning(f"unittest execution failed for {file_path} (return code: {result.returncode})")
                return False
                
        except subprocess.TimeoutExpired:
            logger.error(f"unittest execution timed out for {file_path}")
            results.append(f"unittest timeout ({file_path}): Test execution timed out")
            return False
        except Exception as e:
            logger.error(f"unittest execution error for {file_path}: {e}")
            results.append(f"unittest error ({file_path}): {str(e)}")
            return False
    
    def _run_pytest_tests(self, file_path: str, results: List[str]) -> bool:
        ...
        try:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=90)
            output = f"pytest ({file_path}):\n{result.stdout}{result.stderr}"
            results.append(output)
            
            if result.returncode == 0:
                logger.info(f"pytest execution successful for {file_path}")
                return True
            else:
                logger.warning(f"pytest execution failed for {file_path} (return code: {result.returncode})")
                return False
                
        except subprocess.TimeoutExpired:
            logger.error(f"pytest execution timed out for {file_path}")
            results.append(f"pytest timeout ({file_path}): Test execution timed out")
            return False
        except Exception as e:
            logger.error(f"pytest execution error for {file_path}: {e}")
            results.append(f"pytest error ({file_path}): {str(e)}")
            return False
    
    def _run_fallback_tests(self, file_path: str, results: List[str]) -> None:
        """Run fallback test execution methods."""
        fallback_methods = [
            ("pytest", ["pytest", file_path]),
            ("python unittest", ["python", "-m", "unittest", file_path]),
            ("python direct", ["python", file_path])
        ]
        
        for method_name, cmd in fallback_methods:
            try:
                logger.info(f"Trying fallback method {method_name}: {' '.join(cmd)}")
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=90)
                
                if result.returncode == 0:
                    logger.info(f"Fallback method {method_name} successful for {file_path}")
                    output = f"{method_name} ({file_path}):\n{result.stdout}{result.stderr}"
                    results.append(output)
                    return
                else:
                    logger.warning(f"Fallback method {method_name} failed for {file_path}")
                    
            except Exception as e:
                logger.error(f"Fallback method {method_name} error for {file_path}: {e}")
                continue
        
        # If all fallback methods failed
        results.append(f"All test methods failed for {file_path}")
    
    def _run_original_test_method(self, file_paths: List[str]) -> str:
        """Run tests using the original method as final fallback."""
        logger.info("Using original test runner method")
    ...
    @EnhancedToolManager.tool
    def discover_test_files(self) -> str:
        '''
        Enhanced test file discovery with framework detection and analysis.
        Discovers test files using multiple patterns and analyzes their frameworks.
        Arguments:
            None
        Output:
            Detailed report of discovered test files with framework analysis and recommendations.
        '''
        logger.info("Starting enhanced test file discovery...")
        
        # Discover test files
        test_files = enhanced_test_discovery()
        
        if not test_files:
            return "No test files found in the repository."
        ...
        for file_path in test_files:
            framework = detect_test_framework(file_path)
            test_count = count_test_cases_enhanced(file_path)
        ...
        analysis_results.sort(key=lambda x: x['test_count'], reverse=True)
        
        # Generate report
        report_lines = [
            f"Enhanced Test Discovery Report",
            f"==============================",
            f"",
            f"Total test files found: {len(test_files)}",
            f"Framework distribution: {dict(framework_counts)}",
            f"",
            f"Test Files Analysis:",
            f"-------------------"
        ]
        ...
        for result in analysis_results:
            report_lines.extend([
                ...
                f"  Test cases: {result['test_count']}",
                f"  Size: {result['size']} bytes",
                f""
            ])
        
        # Add recommendations
        primary_framework = max(framework_counts.items(), key=lambda x: x[1])[0]
        report_lines.extend([
            f"Recommendations:",
            f"---------------",
            f"Primary framework: {primary_framework}",
            f"Suggested test runner: {self._get_recommended_runner(primary_framework)}",
            f""
        ])
        
        # Check configuration files
        config_runner, config_mode = detect_from_config_files()
        if config_runner:
            report_lines.extend([
                f"Configuration detected:",
                f"  Runner: {config_runner}",
                f"  Mode: {config_mode}",
                f""
            ])
    ...
    def _get_recommended_runner(self, framework: str) -> str:
        """Get recommended test runner for a framework."""
        if framework == "unittest":
            return "python -m unittest (MODULE mode)"
        elif framework == "pytest":
            return "pytest (FILE mode)"
        else:
            return "pytest (FILE mode) - fallback"
...
def enhanced_test_discovery() -> List[str]:
    """Comprehensive test file discovery with multiple patterns."""
    test_patterns = [
        r'.*test.*\.py$',           # test_*.py, *_test.py
        r'.*tests.*\.py$',          # tests.py, test_suite.py
        r'.*unittest.*\.py$',       # unittest_*.py
        r'.*spec.*\.py$',           # *_spec.py (RSpec style)
    ]
    ...
    for root, _, files in os.walk('.'):
        ...
        for file in files:
            if any(re.match(pattern, file, re.IGNORECASE) for pattern in test_patterns):
                test_files.append(os.path.join(root, file))
    
    return test_files

def detect_test_framework(file_path: str) -> str:
    ...
    unittest_score = sum(1 for pattern in unittest_patterns if re.search(pattern, content))
    pytest_score = sum(1 for pattern in pytest_patterns if re.search(pattern, content))
    
    if unittest_score > pytest_score:
        return "unittest"
    elif pytest_score > 0:
        return "pytest"
    else:
...
def count_test_cases_enhanced(file_path: str) -> int:
    """Count test cases for both unittest and pytest frameworks."""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
    except (FileNotFoundError, UnicodeDecodeError):
    ...
    if framework == "unittest":
        # Count unittest.TestCase methods
        ...
        # Fallback: count any test_* function
        test_functions = re.findall(r'def\s+(test_\w+)', content)
        return len(test_functions)

def detect_from_config_files() -> Tuple[Optional[str], Optional[str]]:
    """Detect test framework from configuration files."""
    config_files = [
        "pytest.ini", "pyproject.toml", "setup.cfg",
        "tox.ini", "conftest.py", "requirements.txt"
    ]
...
def find_custom_test_runner() -> Optional[str]:
    """Find custom test runner scripts in the repository."""
    potential_runners = [
        "run_tests.py", "test_runner.py", "tests.py",
        "test_suite.py", "run_unittests.py"
    ]
    
    for runner in potential_runners:
        if os.path.exists(runner):
            # Check if it's executable and contains test running logic
            try:
                with open(runner, 'r', encoding='utf-8') as f:
                    content = f.read()
                if any(keyword in content.lower() for keyword in ['unittest', 'pytest', 'test', 'run']):
                    return runner
            except (FileNotFoundError, UnicodeDecodeError):
                continue
...
def get_test_runner_and_mode():
    """Enhanced test runner detection with multiple fallback strategies."""
    logger.info("Starting enhanced test runner detection...")
    
    # Strategy 1: Check configuration files first
    config_runner, config_mode = detect_from_config_files()
    if config_runner:
        logger.info(f"Found test framework from config: {config_runner} ({config_mode})")
        return config_runner, config_mode
    
    # Strategy 2: Enhanced test file discovery
    test_files = enhanced_test_discovery()
    logger.info(f"Discovered {len(test_files)} test files: {test_files}")
    
    if not test_files:
        logger.info("No test files found, using default pytest")
        return "pytest", "FILE"
    
    # Strategy 3: Analyze test files to determine framework
    framework_counts = defaultdict(int)
    ...
    for file_path in test_files:
        ...
        logger.info(f"File {file_path} detected as {framework}")
    
    # Strategy 4: Determine primary framework
    primary_framework = max(framework_counts.items(), key=lambda x: x[1])[0]
    logger.info(f"Primary framework detected: {primary_framework} (count: {framework_counts[primary_framework]})")
    
    # Strategy 5: Framework-specific runner selection
    if primary_framework == "unittest":
        # Check for custom test runner
        custom_runner = find_custom_test_runner()
        if custom_runner:
            logger.info(f"Found custom test runner: {custom_runner}")
            return custom_runner, get_test_runner_mode(custom_runner)
        else:
            logger.info("Using python -m unittest for unittest framework")
            return "python -m unittest", "MODULE"
    
    elif primary_framework == "pytest":
        logger.info("Using pytest for pytest framework")
        return "pytest", "FILE"
    
    else:
        # Strategy 6: Mixed or unknown frameworks - try to find the best representative file
        best_file = None
        max_test_count = 0
        ...
        if best_file:
            logger.info(f"Analyzing best test file: {best_file} ({max_test_count} tests)")
            readme_file_path = find_readme(best_file, '.')
            if readme_file_path:
                logger.info(f"README found: {readme_file_path}")
        ...
        logger.info("Using pytest as final fallback")
        return "pytest", "FILE"
...
def fix_task_solve_workflow(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "", n_max_steps = MAX_FIX_TASK_STEPS) -> tuple[str, List[str], List[str]]:
    global run_id
    run_id=run_id_1
    cot=EnhancedCOT()
    tool_manager=FixTaskEnhancedToolManager(
