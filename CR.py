...
ERROR_PREVENTION_PROMPT = textwrap.dedent(
"""
You are debugging error message flow issues. Follow these steps exactly:

**Step 1: Find ALL error messages**
- Read each line of the initial solution
- Identify EVERY single `raise` statement with error messages
- Don't miss any error message, even if content is the same
- Each line with a different `raise` statement is a separate error case

**Step 2: Trace each error case**
- For each `raise` statement found in Step 1:
  - Figure out what condition triggers that specific error
  - Start from the entry point (test case)
  - Trace the exact code execution path step by step
  - Follow the flow to the very end to see what error is actually raised
  - Compare with the expected error from test cases

**Step 3: Fix mismatches**
- If the final raised error doesn't match the expected error:
  - Identify where the error flow goes wrong
  - Fix the code to ensure the correct error is raised
  - Test the fix by tracing the flow again

**Rules:**
- Test EVERY single error case found in Step 1
- Even if error messages have the same content, different lines = different errors
- Each `raise` statement must be tested individually
- Trace the complete execution flow for each error case
- Ensure the final error matches test expectations

**Common issues to look for:**
- try/except blocks that catch correct errors and raise wrong messages
- Error handling that masks the original error
- Multiple error checks that override each other
- Error propagation that changes the message
