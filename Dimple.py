...
8. For reset/refresh operations, ensure that an instance NEVER reuses a value it previously had, even if random seed is reset. Keep track of past values per instance.
    ...
    while (retry < max_retries):
        ...
        try:
            ...
            if problem_type == PROBLEM_TYPE_FIX:
                result = process_fix_task(input_dict, retry)
            else:
...
def process_fix_task(input_dict: Dict[str, Any], retry: int = 0):
...
def fix_task_solve_workflow(problem_statement: str, *, timeout: int, run_id_1: str, instance_id: str = "", \
    test_runner: str = "pytest", test_runner_mode: str = "FILE", n_max_steps = MAX_FIX_TASK_STEPS, retry: int = 0) -> tuple[str, List[str], List[str]]:
    """
    Enhanced workflow with temperature control, rejection tracking, and performance monitoring
    """
    global run_id
    run_id = run_id_1
    # Initialize COT with latest observations to keep
    ...
    logger.info(f"[FIX-TASK] Starting agent execution with enhanced workflow...")
    ...
    logger.info(f"[FIX-TASK] Workflow config: max_steps={n_max_steps}, timeout={timeout}s, run_id={run_id}")
    # Track consecutive rejections and temperature
    consecutive_rejections = 0
    temperature = 0.0
    warning_time_limit = 200
    for step in range(n_max_steps):
        logger.info(f"[FIX-TASK] Step {step + 1}/{n_max_steps}, Elapsed: {time.time() - start_time:.2f}s")
        ...
        if retry > 1:
            messages.append({"role": "system", "content": "Please review the failing tests to identify the issue, then update the code accordingly by applying the necessary fixes through the apply_code_edit tool."})
